function [SRTs conditions] = joergensen2013sim(NSpeechsamples,varargin)
%JOERGENSEN2013SIM Simulate the experiments shown in figure 2 of Jørgensen, Ewert and Dau (2013)
%   Usage: [SRTs conditions]  = joergensen2013sim(NSpeechsamples,varargin)
%
%   Output parameters:
...
%     
%
%   Url: http://amtoolbox.sourceforge.net/doc/speech/joergensen2013sim.php
...
if flags.do_Jetal2013
    ...
    for q = 1:NSpeechsamples
        ...
        for k = 1:length(noise_names)
            ...
            clear tmp tmp2
            [ tmp fs_tmp nbits] = wavread(noise_names{k});
            if fs_tmp ~= fs
...
if flags.do_JandD2011specsub
    ...
    for q = 1:NSpeechsamples
        ...
        clear tmp
        [ tmp fs_tmp nbits] = wavread(noise_name);
        if fs_tmp ~= fs
        ...
        for n = 1:length(conditions)
            ...
            for k = 1:length(SNRs)
                ...
                factor = conditions(n);
                ProcMix  = joergensen2011specsub(test,noise,W,padz,SP,factor);
                
                ProcMixInv = joergensen2011specsub(x-noise,-noise,W,padz,SP,factor);
